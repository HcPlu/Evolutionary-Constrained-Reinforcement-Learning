...
class Tracker(): #Tracker
    ...
    def update(self, updates, generation):
        ...
        Returns:
            None
        """
        self.all_tracker.append([generation]+updates)

        if self.counter % 1 == 0:  # Save to csv file
            filename = self.foldername + self.vars_string[0] + self.project_string
            np.savetxt(filename, np.array(self.all_tracker), fmt='%.3f', delimiter=',')


def add_steps(buffer,trajectory):
