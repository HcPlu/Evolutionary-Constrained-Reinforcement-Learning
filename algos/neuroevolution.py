...
class SSNE:
	...
	def stochastic_ranking(self, rewards, constraints,rcpo_alpha,pf=0.5):
		...
		for i in range(N):
			for j in range(N - 1):
				...
				front = index_I[j]
				back = index_I[j + 1]
				if (self.transformed_phi(constraints[front],rcpo_alpha)==0 and self.transformed_phi(constraints[back],rcpo_alpha)==0) or u < pf:
				# if abs(constraints[front]-constraints[back])<0.0000001 or u < pf:
					...
					if self.transformed_phi(constraints[front],rcpo_alpha) > self.transformed_phi(constraints[back],rcpo_alpha):
						index_I[j] = back
						index_I[j + 1] = front
	...
	def epoch(self, gen, pop, fitness_evals,fitness_constraint, migration,rcpo_alpha):
		...
		#use stochastic_ranking
		if self.args.fitness_rank:
			index_rank = self.list_argsort(fitness_evals)
			index_rank.reverse()
		else:
			index_rank = self.stochastic_ranking(fitness_evals,fitness_constraint,rcpo_alpha)
		...
		for policy in migration:
			...
			leaner_replaces.append(replacee)
		...
		#return removed inidividual
		return leaner_replaces
